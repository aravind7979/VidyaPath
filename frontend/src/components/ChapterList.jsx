...
function ChapterList() {
  ...
  useEffect(() => {
    ...
    fetchChapters();
  }, [uiState.selectedClass, uiState.selectedSubject]);

  if (loading) return <div className="spinner"></div>;

  const icon = SUBJECT_ICONS[uiState.selectedSubject]?.emoji || "📘";

  return (
    <div className="fade-in">
      <div className="text-[28px] font-black text-[#F1F5F9] mb-2">{icon} {uiState.selectedSubject}</div>
      <div className="text-[14px] text-[#64748B] mb-7">Class {uiState.selectedClass} — Select a chapter to begin</div>
      ...
      <div className="flex flex-col gap-3">
        {chapters.map((ch) => (
          <div 
            ...
            onClick={() => setUi({selectedChapter: ch.chapter_name, selectedChapterId: ch.id, currentStep: 'learn', activeMode: null})}
          >
