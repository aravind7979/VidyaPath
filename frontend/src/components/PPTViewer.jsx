...
function PPTViewer({ assets }) {
  const pptAsset = assets.find(a => a.asset_type === 'ppt');
    ...
    <div className="bg-[#1E293B] border border-white/5 rounded-2xl p-8">
      ...
      {!pptAsset ? (
        <div className="border-2 border-dashed border-[#0EA5E9]/30 rounded-2xl p-10 text-center">
          ...
          <div className="text-[#94A3B8] font-bold mb-1">No Presentation Available</div>
          <div className="text-sm text-[#64748B]">A PPT file hasn't been uploaded for this chapter yet.</div>
        </div>
      ) : (
